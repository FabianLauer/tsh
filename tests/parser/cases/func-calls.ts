...
test<[ast.FuncCall]>(
	...
	([$]) => $.parameterList.expressions.length === 0
)
...
test<[ast.BinaryOperation]>(
	...
	([$]) => isInstanceOf($.rightOperand, ast.FuncCall),
	([$]) => (<ast.FuncCall>$.rightOperand).parameterList.expressions.length === 0
)

test<[ast.BinaryOperation]>(
	`a.b(a, b, c, d, e, 1, 2, 3, 4, 5, "abc", "defg", a, b, c)`,
	([$]) => isInstanceOf($, ast.BinaryOperation),
	([$]) => isInstanceOf($.rightOperand, ast.FuncCall),
	([$]) => (<ast.FuncCall>$.rightOperand).parameterList.expressions.length === 15
)
